...
import { Command, CreditCard, LayoutTemplate, Settings, User } from "lucide-react";
		...
		<section className="px-5 py-24 sm:px-8 lg:px-10 lg:py-32 relative overflow-hidden">
			...
			<div className="mx-auto max-w-344">
				<div className="text-center mb-16">
					...
					>
						Inspired by top-tier modern UI systems, embedded directly for an elegant visual experience.
					</motion.p>
				</div>
				...
				>
					...
					<motion.div variants={itemVariants} className="interactive-surface rounded-[24px] border border-[#171717]/8 bg-white/60 p-6 shadow-[0_18px_45px_rgba(23,23,23,0.02)] dark:border-white/8 dark:bg-white/4 backdrop-blur-sm">
						<div className="text-sm font-medium tracking-tight mb-5 dark:text-[#f3f4f1]">Interactive Controls</div>
						<div className="grid gap-4">
							...
							<div className="flex items-center justify-between p-3 rounded-xl border border-[#171717]/8 bg-white/50 dark:border-white/6 dark:bg-[#111315]/50 mt-2">
								<div className="text-sm dark:text-[#f3f4f1]">Push Notifications</div>
								<button 
                                    type="button"
					...
					<motion.div variants={itemVariants} className="interactive-surface rounded-[24px] border border-[#171717]/8 bg-white/60 p-6 shadow-[0_18px_45px_rgba(23,23,23,0.02)] dark:border-white/8 dark:bg-white/4 backdrop-blur-sm lg:col-span-1 md:col-span-1">
						<div className="text-sm font-medium tracking-tight mb-5 dark:text-[#f3f4f1]">Command Menu</div>
						...
						<div className="rounded-xl border border-[#171717]/8 bg-white overflow-hidden dark:border-white/8 dark:bg-[#15181b] shadow-sm">
							<div className="p-2 border-b border-[#171717]/8 dark:border-white/8">
								<input className="w-full bg-transparent px-2 py-1 text-sm outline-none dark:text-white" placeholder="Type a command or search..." />
							</div>
							<div className="p-2 space-y-1">
								...
								{[
									...
									<button type="button" key={item.label} className={`flex w-full items-center gap-2 rounded-lg px-2 py-1.5 text-sm transition-colors ${i === 0 ? 'bg-teal-500/10 text-teal-700 dark:text-teal-300' : 'text-[#171717] hover:bg-[#171717]/5 dark:text-[#f3f4f1] dark:hover:bg-white/5'}`}>
					...
					<motion.div variants={itemVariants} className="interactive-surface rounded-[24px] border border-[#171717]/8 bg-white/60 p-6 shadow-[0_18px_45px_rgba(23,23,23,0.02)] dark:border-white/8 dark:bg-white/4 backdrop-blur-sm md:col-span-2 lg:col-span-1">
						<div className="text-sm font-medium tracking-tight mb-5 dark:text-[#f3f4f1]">Data Display</div>
						...
						<div className="rounded-[18px] border border-[#171717]/8 bg-white p-4 dark:border-white/8 dark:bg-[#131619] shadow-sm">
							<div className="flex items-center gap-3 border-b border-[#171717]/8 pb-4 dark:border-white/8">
								...
								<div>
									<div className="text-sm font-semibold dark:text-[#f3f4f1]">John Doe</div>
									<div className="text-[11px] text-[#5f6258] dark:text-[#a6aca6]">Pro Member</div>
								</div>
							</div>
							
							<div className="mt-4 flex gap-2 border-b border-[#171717]/8 pb-4 dark:border-white/8">
								<button type="button" onClick={() => setActiveTab(0)} className={`text-xs font-medium px-3 py-1.5 rounded-full transition-colors ${activeTab === 0 ? 'bg-[#111315] text-white dark:bg-white dark:text-[#111315]' : 'text-[#5f6258] hover:bg-[#171717]/5 dark:text-[#a6aca6] dark:hover:bg-white/5'}`}>
									Overview
								</button>
								<button type="button" onClick={() => setActiveTab(1)} className={`text-xs font-medium px-3 py-1.5 rounded-full transition-colors ${activeTab === 1 ? 'bg-[#111315] text-white dark:bg-white dark:text-[#111315]' : 'text-[#5f6258] hover:bg-[#171717]/5 dark:text-[#a6aca6] dark:hover:bg-white/5'}`}>
									Activity
								</button>
							</div>
								...
								{activeTab === 0 ? (
									<div className="space-y-3">
										<div className="flex justify-between items-center text-sm">
											<span className="text-[#5f6258] dark:text-[#a6aca6]">Workspace usage</span>
										...
										{[1, 2].map((i) => (
											<div key={i} className="flex gap-3 text-sm">
												...
												<div className="dark:text-[#dbe1dc] leading-tight text-xs">
													Generated a new <span className="font-semibold text-teal-600 dark:text-teal-400">code artifact</span> for authentications layer.
												</div>
											</div>
										))}
									</div>
