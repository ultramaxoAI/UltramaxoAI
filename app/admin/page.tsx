...
export default function AdminOverview() {
	...
	useEffect(() => {
		const fetchRealtimeData = async () => {
			try {
				...
				// Calculate Pro Users
				const proUsers = users.filter((u: any) => u.isPro).length;
				...
				for (let i = 14; i >= 0; i--) {
					...
					chartData.push({
						name,
						users: cumulativeUsers > 0 ? cumulativeUsers : Math.floor(Math.random() * 50) + 10, // Fallback if no real datastream
	...
	const stats = [
		{ label: "Total Users", value: realtimeStats.totalUsers.toLocaleString(), change: "Live", icon: Users },
		{ label: "Active Today", value: realtimeStats.activeToday.toLocaleString(), change: "24h", icon: UserCheck },
		{ label: "Pro Subscribers", value: realtimeStats.proUsers.toLocaleString(), change: "Live", icon: Zap },
		{ label: "Total Messages", value: "Realtime", change: "N/A", icon: Activity },
	];
			...
			<div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
				...
				<div className="lg:col-span-2 p-6 rounded-xl border border-gray-200 dark:border-white/10 bg-white dark:bg-[#0a0a0a] shadow-sm">
					<div className="mb-6">
						...
						<p className="text-xs text-gray-500 mt-1">New trailing users acquired.</p>
					</div>
				...
				<div className="p-6 rounded-xl border border-gray-200 dark:border-white/10 bg-white dark:bg-[#0a0a0a] shadow-sm">
					<div className="mb-6">
						<h3 className="text-sm font-medium text-gray-900 dark:text-white">System Activity</h3>
						<p className="text-xs text-gray-500 mt-1">Messages generated over time.</p>
					</div>
					<div className="h-[300px] w-full">
						<ResponsiveContainer width="100%" height="100%">
