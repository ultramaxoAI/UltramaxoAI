      ...
      <div className="bg-zinc-900 border-b border-zinc-800 px-4 py-2 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <button 
            ...
            disabled={!previewUrl}
            className="p-1 hover:bg-zinc-800 rounded text-zinc-400 hover:text-zinc-200 transition-colors disabled:opacity-30"
          >
            <RotateCw size={14} />
          </button>
          
          {/* Address bar input */}
          <div className="bg-zinc-950 border border-zinc-800 rounded px-3 py-1 text-xs text-zinc-400 font-mono w-[300px] flex items-center justify-between">
        ...
        {/* Viewport size togglers */}
        <div className="flex items-center gap-1">
          <button
            onClick={() => setViewport('desktop')}
            className={`p-1.5 rounded transition-colors ${
              viewport === 'desktop' ? 'bg-purple-600/30 text-purple-400' : 'text-zinc-500 hover:text-zinc-300'
            }`}
            title="Desktop View"
          >
            <Monitor size={15} />
          ...
          <button
            ...
            className={`p-1.5 rounded transition-colors ${
              viewport === 'tablet' ? 'bg-purple-600/30 text-purple-400' : 'text-zinc-500 hover:text-zinc-300'
            }`}
            title="Tablet View"
          >
            <Tablet size={15} />
          ...
          <button
            ...
            className={`p-1.5 rounded transition-colors ${
              viewport === 'mobile' ? 'bg-purple-600/30 text-purple-400' : 'text-zinc-500 hover:text-zinc-300'
            }`}
            title="Mobile View"
          >
            <Smartphone size={15} />
      ...
      <div className="flex-1 bg-zinc-900/50 p-4 flex items-center justify-center overflow-auto">
        {previewUrl ? (
          <div 
            ...
            className="bg-white rounded-lg shadow-2xl border border-zinc-800 overflow-hidden"
          >
            <iframe
              ref={iframeRef}
              src={previewUrl}
          ...
          <div className="w-full max-w-2xl h-full flex flex-col justify-center">
            {/* Terminal logs showing build status */}
            <div className="text-center mb-6">
              ...
              <p className="text-zinc-300 text-sm font-medium">Booting Sandbox & Dev Server...</p>
              <p className="text-zinc-500 text-xs mt-1">This might take a minute during npm install.</p>
            </div>
            
            <div className="bg-zinc-950 border border-zinc-800 rounded-lg p-4 font-mono text-[11px] text-zinc-400 h-[250px] overflow-y-auto shadow-inner">
