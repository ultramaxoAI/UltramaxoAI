...
export const useProjectStore = create<ProjectState>((set, get) => ({
  ...
  logout: () => {
    set({ user: null, token: null, activeProject: null, activeFile: null, projects: [] });
    localStorage.removeItem('auth_token');
    localStorage.removeItem('auth_user');
  },
  ...
  setActiveProject: (project) => {
    ...
    if (project) {
      ...
      // Cache project in localforage for offline access
  ...
  syncOfflineChanges: async () => {
    const { activeProject, unsyncedChanges, token, isOnline } = get();
    if (!activeProject || !token || !isOnline) return;
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.log('Project synchronized successfully with Cloud');
      }
    } catch (err) {
